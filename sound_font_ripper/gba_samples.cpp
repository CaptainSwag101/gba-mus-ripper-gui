...
int32_t GBASamples::build_sample(uint32_t pointer)
{	// Do nothing if sample already exists
	for(int32_t i=samples_list.size()-1; i >= 0; --i)
	...
	uint32_t pitch_correction = int32_t((int_delta_note - delta_note) * 100);
	uint32_t original_pitch = 60 + (int32_t)int_delta_note;
		...
		switch(type)
		{
			case 0:		// Square wave
			{
				...
				{	// Square wave with constant duty cycle
					uint32_t base_pointer = 128 + 64 * (duty_cycle >> 2);
...
int32_t GBASamples::build_GB3_samples(uint32_t pointer)
{
	// Do nothing if sample already exists
	for(int32_t i=samples_list.size()-1; i >= 0; --i)
		if (samples_list[i] == pointer) return i;
	...
	for(int32_t i=4; i!=0; --i) samples_list.push_back(pointer);
	return samples_list.size() - 1;
}

//Build square wave sample
int32_t GBASamples::build_pulse_samples(uint32_t duty_cycle)
{	// Do nothing if sample already exists
	for(int32_t i=samples_list.size()-1; i >= 0; --i)
	...
	const int32_t loop_size[5] = {689, 344, 172, 86, 43};
...
int32_t GBASamples::build_noise_sample(bool metallic, int32_t key)
{
	//prevent out of range keys
	if (key < 42) key = 42;
	if (key > 77) key = 76;

	uint32_t num = metallic ? 3 + (key-42) : 80 + (key-42);
	...
	const int32_t metallic_len_tbl[] =
