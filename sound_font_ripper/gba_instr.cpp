...
void GBAInstr::generate_adsr_generators(const uint32_t adsr)
{
	...
	int32_t attack = adsr & 0xFF;
    int32_t decay = (adsr >> 8) & 0xFF;
    int32_t sustain = (adsr >> 16) & 0xFF;
    int32_t release = adsr >> 24;
...
int32_t GBAInstr::build_sampled_instrument(const inst_data inst)
{
	...
	int32_t sample_index = samples.build_sample(sample_pointer);
...
int32_t GBAInstr::build_every_keysplit_instrument(const inst_data inst)
{
	// Do nothing if this instrument already exists !
	inst_it it = inst_map.find(inst);
	if (it != inst_map.end()) return (*it).second;
	...
	for(int32_t key = 0; key < 128; key ++)
	{
		try
		{
			...
			int32_t instrType = fgetc(inGBA);		// Instrument type
			int32_t keynum = fgetc(inGBA);			// Key (every key split instrument only)
		/*  int32_t unused_byte =*/ fgetc(inGBA);		// Unknown/unused byte
			int32_t panning = fgetc(inGBA);			// Panning (every key split instrument only)
			...
			switch(instrType & 0x0f)
			{
				...
				case 0:
				{
					...
					int32_t rootkey = 60 + int32_t(round(delta_note));
			...
			if (panning != 0)
                sf2->add_new_inst_generator(SFGenerator::pan, int32_t((panning - 192) * (500 / 128.0)));
...
int32_t GBAInstr::build_keysplit_instrument(const inst_data inst)
{
	// Do nothing if this instrument already exists !
	inst_it it = inst_map.find(inst);
	if (it != inst_map.end()) return (*it).second;
    ...
    for(uint32_t i = 0; i < index_list.size(); i++)
	{
		try
		{
			...
			int32_t inst_type = fgetc(inGBA);		// Instrument type
		 /* int32_t keynum = */ fgetc(inGBA);		// Key (every key split instrument only)
		 /* int32_t unused_byte = */ fgetc(inGBA);	// Unknown/unused byte
		 /* int32_t panning = */ fgetc(inGBA);		// Panning (every key split instrument only)
...
int32_t GBAInstr::build_GB3_instrument(const inst_data inst)
{
	// Do nothing if this instrument already exists !
	inst_it it = inst_map.find(inst);
	if (it != inst_map.end()) return (*it).second;
	...
	int32_t sample = samples.build_GB3_samples(sample_pointer);
...
int32_t GBAInstr::build_pulse_instrument(const inst_data inst)
{
	// Do nothing if this instrument already exists !
	inst_it it = inst_map.find(inst);
	if (it != inst_map.end()) return (*it).second;

	uint32_t duty_cycle = inst.word1;
	...
	int32_t sample = samples.build_pulse_samples(duty_cycle);
...
int32_t GBAInstr::build_noise_instrument(const inst_data inst)
{
	// Do nothing if this instrument already exists !
	inst_it it = inst_map.find(inst);
	if (it != inst_map.end()) return (*it).second;
	...
	sf2->add_new_inst_bag();
	int32_t sample42 = samples.build_noise_sample(metallic, 42);
	sf2->add_new_inst_generator(SFGenerator::keyRange, 0, 42);
	sf2->add_new_inst_generator(SFGenerator::sampleModes, 1);
	sf2->add_new_inst_generator(SFGenerator::sampleID, sample42);

    for(int32_t key = 43; key <= 77; key++)
	{
		sf2->add_new_inst_bag();
		int32_t sample = samples.build_noise_sample(metallic, key);
		sf2->add_new_inst_generator(SFGenerator::keyRange, key, key);
		sf2->add_new_inst_generator(SFGenerator::sampleModes, 1);
		sf2->add_new_inst_generator(SFGenerator::sampleID, sample);		
	}
	...
	sf2->add_new_inst_bag();
	int32_t sample78 = samples.build_noise_sample(metallic, 78);
