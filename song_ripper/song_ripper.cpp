...
static uint32_t parseArguments(const int argc, std::string args[])
{
    if (argc < 4) print_instructions();

    printf("Opening file: %s \n", args[1].c_str());

	// Open the input and output files
    inGBA = fopen(args[1].c_str(), "rb");
    if(!inGBA)
	{
    ...
    for(int i = 4; i < argc; i++)
	{
        ...
        if(args[i].c_str()[0] == '-')
		{
            if(args[i].c_str()[1] == 'b')
			{
                if(strlen(args[i].c_str()) < 3) print_instructions();
                bank_number = atoi(args[i].c_str() + 2);
				bank_used = true;
			}
            else if(args[i].c_str()[1]=='r' && args[i].c_str()[2]=='c')
				rc = true;
            else if(args[i].c_str()[1]=='g' && args[i].c_str()[2]=='s')
				gs = true;
            else if(args[i].c_str()[1]=='x' && args[i].c_str()[2]=='g')
				xg = true;
            else if(args[i].c_str()[1]=='l' && args[i].c_str()[2]=='v')
				lv = true;
            else if(args[i].c_str()[1]=='s' && args[i].c_str()[2]=='v')
				sv = true;
			else
				print_instructions();
		}
    ...
    return strtoul(args[3].c_str(), 0, 0);
}
